...
def _build_wf(job, n):
    ...
    # Anna's quality-experiment knobs (per-job overrides; defaults preserve current behavior).
    # Drop-in from HANDOFF_REPLY_TO_ANNA.md section 5 — PR back to upstream pending.
    if job.get("shift") is not None:    wf["4"]["inputs"]["shift"]       = float(job["shift"])
    wf["14"]["inputs"]["strength"] = float(job.get("strength", 1.0))
    if job.get("seed") is not None:     wf["15"]["inputs"]["noise_seed"] = int(job["seed"])
...
def _boot_warmup():
    ...
    try:
        ...
        try:
            ...
            _beacon("warmup_validation_failed", code=he.code, detail=he.read().decode()[:1500])
            return
