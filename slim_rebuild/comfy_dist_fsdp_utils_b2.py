...
# PINNED STAGING (2026-07-13): per-tensor pageable .to(device) from mmap-backed CPU tensors ran at
# 260-450 MB/s on some hosts (worker qhrwx833p2gtb9 telemetry: recon+gpucopy 19.8s vs 1.8-2.0s on
# prod-class, ~7.5ms/op driver overhead — NUMA/IOMMU/pageable-path class, rival undiscriminated and
# doesn't need to be: pinned bounce + DMA beats all of them). Reusable pinned buffer, synchronous
# per tensor (the buffer is reused, so DMA must complete before the next memcpy — per-op sync on
# pinned DMA is us-scale, not the ms-scale pageable hit). VALIDATED 7/13 (worker 0lfvy7a3f9s9ld):
# recon+gpucopy 19.8/20.0s -> 3.7/1.9s, PRESHARD_VERIFY 17/17 PASS both ranks, PARAM_CHECKSUM
# exact. Gate: default ON everywhere; FSDP_PINNED_STAGE=0 is the kill-switch.
_PINSTAGE_ENV = _ps_os.environ.get("FSDP_PINNED_STAGE", "auto")
_PINSTAGE_ON = _PINSTAGE_ENV != "0"
_PIN_BUF = None
_PIN_OK = True

def _pinned_to_device(t, device):
    global _PIN_BUF, _PIN_OK
    if not _PIN_OK or not torch.is_tensor(t) or t.device.type != "cpu" or t.numel() == 0:
        return t.to(device)
    try:
        src = t.contiguous().view(-1).view(torch.uint8)
        nb = src.numel()
        cap = _PIN_BUF.numel() if _PIN_BUF is not None else 0
        if nb > cap and cap < (1 << 30):
            _PIN_BUF = None
            _PIN_BUF = torch.empty(min(max(nb, 64 << 20), 1 << 30), dtype=torch.uint8, pin_memory=True)
            cap = _PIN_BUF.numel()
        if nb <= cap:
            stage = _PIN_BUF[:nb]
            stage.copy_(src)
            out = stage.to(device, non_blocking=False)
        else:  # tensor larger than the 1 GiB buffer: chunk through it
            out = torch.empty(nb, dtype=torch.uint8, device=device)
            for off in range(0, nb, cap):
                n = min(cap, nb - off)
                _PIN_BUF[:n].copy_(src[off:off + n])
                out[off:off + n].copy_(_PIN_BUF[:n], non_blocking=False)
        return out.view(t.dtype).view(t.shape)
    except Exception as _e:
        _PIN_OK = False
        print(f"[b2] pinned-stage disabled ({_e!r}) -> pageable .to() fallback", flush=True)
        return t.to(device)

def _dev_copy(t, device):
    return _pinned_to_device(t, device) if _PINSTAGE_ON else t.to(device)
    ...
    if entry['q']:
        ...
        try:
            ...
            if _dc.is_dataclass(p):
                _upd = {f.name: _dev_copy(getattr(p, f.name), device)
            ...
            p = entry['p']
        return QuantizedTensor(_dev_copy(entry['qd'], device), cls, p)
    return _dev_copy(entry['d'], device)
...
def load_fsdp_shards(diffusion_model, path, device):
    ...
    def _core_probe():
        # WHERE and HOW FAST is this thread right now: (cpu_id, live MHz, physical socket).
        # Discriminates the 2026-07-14/15 single-thread slowdowns: NUMA-remote work vs a slow core.
        # All best-effort; -1 marks "unreadable" (cpufreq sysfs is often absent in containers).
        _cpu = -1
        try: _cpu = _ps_os.sched_getcpu()
        except Exception as _e:
            print(f"[b2][core_probe] sched_getcpu failed: {_e!r}", flush=True)
        if _cpu < 0:  # fallback: raw getcpu(2) — 7/15 first boot returned (-1,-1,-1) on all ranks
            try:
                import ctypes as _ct
                _c, _n = _ct.c_uint(0), _ct.c_uint(0)
                _lc = _ct.CDLL("libc.so.6", use_errno=True)
                if _lc.syscall(309, _ct.byref(_c), _ct.byref(_n), None) == 0:  # __NR_getcpu x86_64
                    _cpu = _c.value
            except Exception as _e:
                print(f"[b2][core_probe] getcpu syscall failed: {_e!r}", flush=True)
        if _cpu < 0: return (-1, -1, -1)
        _mhz = -1
        try:
            with open(f"/sys/devices/system/cpu/cpu{_cpu}/cpufreq/scaling_cur_freq") as _f:
                _mhz = int(_f.read().strip()) // 1000
        except Exception:
            try:  # fallback: /proc/cpuinfo block for this processor
                _blk = -1
                for _l in open("/proc/cpuinfo"):
                    if _l.startswith("processor"): _blk = int(_l.split(":")[1])
                    elif _blk == _cpu and _l.startswith("cpu MHz"):
                        _mhz = int(float(_l.split(":")[1])); break
            except Exception: pass
        _pkg = -1
        try:
            with open(f"/sys/devices/system/cpu/cpu{_cpu}/topology/physical_package_id") as _f:
                _pkg = int(_f.read().strip())
        except Exception: pass
        return (_cpu, _mhz, _pkg)
    def _numa_of_path(_p):
        # /proc/self/numa_maps rows for the mapped shard file -> pages per NUMA node.
        # Read AFTER torch.load has the file mapped. {} = no mapping found / unreadable.
        _out = {}
        try:
            import re as _re
            _base = _ps_os.path.basename(_p)
            for _l in open("/proc/self/numa_maps"):
                if _base in _l:
                    for _n, _v in _re.findall(r"\bN(\d+)=(\d+)", _l):
                        _out[f"N{_n}"] = _out.get(f"N{_n}", 0) + int(_v)
        except Exception: pass
        return _out
    ...
    _cp_loaded = _core_probe()
    _numa = _numa_of_path(path) if _use_mmap else {}
    ...
    _cp_recon = _core_probe()
    # numa_maps only shows FAULTED pages — mmap is lazy, so the meaningful read is here,
    # after recon touched every tensor (the post-torch.load read is the near-empty baseline)
    _numa2 = _numa_of_path(path) if _use_mmap else {}
    ...
    _bd = (f"[Rank {rank}] PRESHARD_LOAD_BREAKDOWN mode={_mode} pinned_stage={_PINSTAGE_ON and _PIN_OK} "
           f"parallel_read={_t_pread:.1f}s cache_delta={_cdelta:.1f}GiB "
           ...
           f"core[start/loaded/recon]={_cp_start}/{_cp_loaded}/{_cp_recon}(cpu,MHz,pkg) "
           f"numa_post_load={_numa} numa_post_recon={_numa2}")
