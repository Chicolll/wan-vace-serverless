...
VOL      = "/runpod-volume/native-xdit"           # network volume mount on serverless
MODEL    = f"{VOL}/model/Wan2.1-VACE-14B"
...
def handler(event):
    ...
    if not os.path.exists(f"{MODEL}/config.json"):
        return {"error": f"model not found at {MODEL} — is the network volume attached?"}
