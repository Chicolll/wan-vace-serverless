...
from .fsdp_utils import save_fsdp_shards, load_fsdp_shards, preshard_version  # PRESHARD-CACHE (b2)
...
def patch_fsdp(self):
    print(f"[Rank {self.rank}] Applying FSDP to {type(self.model.diffusion_model).__name__} "
          f"[PRESHARD-CACHE active, file={__file__}]", flush=True)
    ...
    # FSDP_PHASES timers (2026-07-14 audit): the registration path had NO per-step timing, so the
    # checksum / traversal / wrap budget was guesswork. One summary line per rank at the end.
    import time as _pht
    _ph, _phm = {}, _pht.time()
    def _phase(name):
        nonlocal _phm
        _ph[name] = round(_pht.time() - _phm, 2); _phm = _pht.time()
    ...
    # ── PRESHARD CACHE (b2): skip the ~130s per-tensor fp8 scatter by loading a
    # per-rank shard checkpoint from disk when one exists for THIS exact config.
    # Version key encodes world_size + full state-dict structure + a value fingerprint,
    # so any model/LoRA/fp8/gpuCount change → different path → miss → safe full scatter.
    import os as _os, time as _t
    _ws = dist.get_world_size() if dist.is_initialized() else 1
    _sdir = _os.environ.get("FSDP_SHARD_DIR", "") or "/runpod-volume/fsdp_shards"
    _spath = ""
    if _sdir:
        try:
            _ver = preshard_version(self.fsdp_state_dict, _ws)
            _spath = _os.path.join(_sdir, _ver, f"rank_{self.rank}.pt")
        except Exception as _e:
            print(f"[Rank {self.rank}] PRESHARD version-key failed ({_e}); scatter only", flush=True)
            _spath = ""
    _loaded_preshard = False
    if _spath and _os.path.isfile(_spath):
        try:
            _t0 = _t.time()
            load_fsdp_shards(diffusion_model, _spath, target_device)
            print(f"[Rank {self.rank}] PRESHARD load {_spath} in {_t.time()-_t0:.1f}s (skipped full scatter)", flush=True)
            _loaded_preshard = True
        except Exception as _e:
            print(f"[Rank {self.rank}] PRESHARD load FAILED ({_e}); falling back to full scatter", flush=True)

    if not _loaded_preshard:
        if use_quant_loader:
        ...
        # Save shards for next time (atomic; non-fatal on failure).
        if _spath:
            try:
                save_fsdp_shards(diffusion_model, _spath, self.rank, _ws)
            except Exception as _e:
                print(f"[Rank {self.rank}] PRESHARD save failed (non-fatal): {_e}", flush=True)

    self.fsdp_state_dict = None
    _phase("load")

    # CORRECTNESS CHECK: checksum the local sharded params. R1(scatter) vs R3(preshard) must MATCH.
    # Env-gated (2026-07-14 audit): ~1331 .item() GPU syncs per rank + a volume append on EVERY boot,
    # in the critical path; byte-integrity is already covered by artifact fingerprints. Default stays
    # ON until the measurement boot prices it; FSDP_PARAM_CHECKSUM=0 skips.
    if _os.environ.get("FSDP_PARAM_CHECKSUM", "1") != "0":
        try:
            _ck = 0.0; _nc = 0
            for _n, _p in diffusion_model.named_parameters():
                _lt = _p._local_tensor if hasattr(_p, "_local_tensor") else (_p.data if hasattr(_p, "data") else _p)
                _q = getattr(_lt, "_qdata", None)
                _t = _q if _q is not None else _lt
                _ck += float(_t.detach().float().abs().sum().item()); _nc += 1
            _msg = (f"PARAM_CHECKSUM rank={self.rank} sum={_ck:.6e} n={_nc} "
                    f"dur_s={_pht.time()-_phm:.2f} preshard={_loaded_preshard} spath={_spath}")
            print(f"[Rank {self.rank}] {_msg}", flush=True)
            try:  # ALSO write to volume so it's readable even if comfy.log S3-lags
                import os as _o2
                with open("/runpod-volume/preshard_ck_log.txt", "a") as _f:
                    _f.write(f"{_o2.getpid()} {_msg}\n")
            except Exception:
                pass
        except Exception as _e:
            print(f"[Rank {self.rank}] PARAM_CHECKSUM failed: {_e}", flush=True)
    else:
        print(f"[Rank {self.rank}] PARAM_CHECKSUM skipped (FSDP_PARAM_CHECKSUM=0)", flush=True)
    _phase("checksum")

    _pre_init_fsdp(diffusion_model)
    _phase("preinit")

    print(f"[Rank {self.rank}] FSDP_PHASES " +
          " ".join(f"{k}={v}s" for k, v in _ph.items()) +
          f" total={sum(_ph.values()):.2f}s", flush=True)
