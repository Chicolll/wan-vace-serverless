...
# ============================================================================
# RAYLIGHT RAY PRE-START PATCH  (appended by wan-vace-fsdp; gated by RAYLIGHT_PRESTART=1)
# Shipped as code_hotpatch/raylight_nodes_r1.py, installed over this file BEFORE ComfyUI boots.
#
# WHY: RayInitializer.spawn_actor (ray.init + spawn N GPU actors + per-worker torch import + CUDA
# context + NCCL rendezvous + USP groups) is model-INDEPENDENT, and its actors are long-lived and
# reused across prompts. Today it runs serially on the render critical path (~6-10s). This patch
# (1) MEMOIZES spawn_actor so a repeat call with the same topology returns the LIVE actors instead
# of raylight's default ray.shutdown()+ray.init()+respawn, and (2) fires spawn_actor ONCE on a boot
# thread so the Ray boot overlaps ComfyUI import + shard prewarm. The warmup /prompt then hits the
# memo and goes straight to model load.
#
# HARDENED AFTER RED-TEAM:
#  F1  a BUSY single-threaded actor (mid model-load) times out the __ray_ready__ probe but is ALIVE.
#      Treating it as dead would rebuild, and the rebuild's ray.shutdown() would kill an in-flight
#      render. => timeout/unknown == alive; ONLY a definitive RayActorError rebuilds. Plus the boot
#      builder NO-OPS if actors already exist (never destroy over a live session).
#  F2  the boot config is DERIVED from the same workflow JSON + the same _build_wf overwrites the
#      handler uses, so the memo key is byte-identical to the warmup node's key (no hardcode drift
#      that would silently no-op or re-arm the shutdown).
#  F3  the warmup waits for the lock with a TIMEOUT and fails loud (handler treats a warmup failure
#      as non-fatal; the real render then loads normally) instead of blocking forever on a stuck build.
#  F4  clear_vram_after_sampling + use_mmap are in the key (a differing node value forces a rebuild
#      rather than silently inheriting the boot value).
#  F6  the boot build is delayed slightly so it doesn't race ComfyUI's own CUDA init.
#
# SAFETY: fully env-gated. Flag unset => the boot thread never fires and the memo is never populated
# (the wrapper is a transparent pass-through: MISS -> original). A topology mismatch or ANY failure
# => memo miss => the ORIGINAL spawn_actor runs (today's behavior). The lock + the "skip if actors
# exist" guard ensure the boot build and a racing warmup /prompt never both reach ray.shutdown().
# ============================================================================
import threading as _ps_th, inspect as _ps_inspect, traceback as _ps_tb, time as _ps_time, json as _ps_json

_PRESTART_LOCK = _ps_th.RLock()
_PRESTART_MEMO = {}                        # topology_key -> [ray_actors, ray_actor_fn]
_PRESTART_LOCK_WAIT_S = float(os.environ.get("RAYLIGHT_PRESTART_LOCK_WAIT_S", "180"))
_PRESTART_KEYS = ("ray_cluster_address", "ray_cluster_namespace", "GPU", "ulysses_degree",
                  "ring_degree", "cfg_degree", "dp_degree", "sync_ulysses", "clear_vram_after_sampling",
                  "FSDP", "FSDP_CPU_OFFLOAD", "XFuser_attention", "use_mmap", "GPU_SELECT")
_ORIG_SPAWN_ACTOR = RayInitializer.spawn_actor
_SPAWN_SIG = _ps_inspect.signature(_ORIG_SPAWN_ACTOR)


def _prestart_actors_alive(payload):
    """A cached actor is DEAD only if it definitively died. A BUSY actor (max_concurrency=1, mid
    model-load) times out the __ray_ready__ probe yet is ALIVE -- treating it as dead would rebuild
    and the rebuild's ray.shutdown() would kill an in-flight render. So timeout/unknown => alive."""
    try:
        actors = payload[0]
        workers = actors.get("workers") if isinstance(actors, dict) else None
    except Exception:
        return False
    if not workers:
        return False
    ...
    for a in workers:
        try:
            ray.get(a.__ray_ready__.remote(), timeout=5)
        except GetTimeoutError:
            continue          # single-threaded actor is busy == ALIVE
        except RayActorError:
            return False      # actor truly died -> allow rebuild
        except Exception:
            continue          # unknown -> assume alive; NEVER ray.shutdown() a live session on a probe
    return True


def _prestart_key(self, args, kw):
    try:
        bound = _SPAWN_SIG.bind(self, *args, **kw)
        bound.apply_defaults()
        return tuple(bound.arguments.get(k) for k in _PRESTART_KEYS)
    except Exception:
        return None


def _prestart_memoized_spawn_actor(self, *args, **kw):
    key = _prestart_key(self, args, kw)
    got = _PRESTART_LOCK.acquire(timeout=_PRESTART_LOCK_WAIT_S)
    if not got:
        # A boot build is stuck holding the lock (rare: NCCL/TCP-store hang). Fail LOUD rather than
        # block the render forever; the handler treats a warmup failure as non-fatal and the real
        # render loads normally on the lazy path.
        raise RuntimeError("[Raylight][PRESTART] timed out acquiring prestart lock (stuck build?)")
    try:
        if key is not None:
            payload = _PRESTART_MEMO.get(key)
            if payload is not None and _prestart_actors_alive(payload):
                print("[Raylight][PRESTART] spawn_actor memo HIT -> reusing live actors", flush=True)
                return (payload,)
        print("[Raylight][PRESTART] spawn_actor memo MISS -> building actors (ray.init + spawn)", flush=True)
        result = _ORIG_SPAWN_ACTOR(self, *args, **kw)      # the real ray.shutdown()+ray.init()+respawn
        if key is not None:
            _PRESTART_MEMO.clear()                          # only one topology alive at a time
            _PRESTART_MEMO[key] = result[0]
        return result
    finally:
        _PRESTART_LOCK.release()


RayInitializer.spawn_actor = _prestart_memoized_spawn_actor


def _prestart_cfg_from_workflow():
    """Derive the pre-start topology from the SAME workflow JSON + the SAME overwrites the handler's
    _build_wf applies, so the boot memo key is byte-identical to the warmup node's key."""
    import torch as _t
    wf = _ps_json.load(open(os.environ["WF_PATH"]))
    inp = dict(wf["1"]["inputs"])                                    # the RayInitializer node inputs
    n = int(os.environ.get("RAYLIGHT_PRESTART_GPU", "0")) or _t.cuda.device_count()
    inp["GPU"] = n                                                   # _build_wf: GPU = n_gpus
    inp["ulysses_degree"] = n                                        # _build_wf: ulysses_degree = n_gpus
    inp["FSDP"] = (os.environ.get("RAYLIGHT_PRESTART_FSDP", "1") == "1")   # warmup job -> fsdp default True
    inp["FSDP_CPU_OFFLOAD"] = False                                  # _build_wf
    inp["clear_vram_after_sampling"] = False                        # _build_wf
    valid = set(_SPAWN_SIG.parameters) - {"self"}
    return {k: v for k, v in inp.items() if k in valid}, n


def _prestart_boot():
    """Build the Ray actors at boot (off the render critical path). NO-OPS if actors already exist,
    so it can never ray.shutdown() over a live session. Non-fatal: any failure falls back to today's
    lazy build during the warmup render."""
    try:
        _ps_delay = float(os.environ.get("RAYLIGHT_PRESTART_DELAY_S", "3"))       # let ComfyUI's own CUDA init settle (F6)
        print(f"[Raylight][PRESTART] settle delay {_ps_delay}s", flush=True)      # measured 7/13: the delay ran serial after comfy import
        _ps_time.sleep(_ps_delay)
        try:
            # one-time wheel fingerprint (2026-07-09): the driver logged the rpc_client.h:204 fatal
            # line yet SURVIVED — official wheels _Exit there unconditionally. Identify the binary.
            _so = os.path.join(os.path.dirname(ray.__file__), "_raylet.so")
            _n = open(_so, "rb").read().count(b"The program will terminate") if os.path.isfile(_so) else -1
            print(f"[Raylight][PRESTART] wheel fingerprint: ray={ray.__version__} file={ray.__file__} exit_strings={_n}", flush=True)
        except Exception as _fe:
            print(f"[Raylight][PRESTART] wheel fingerprint failed: {_fe!r}", flush=True)
        # F7 (root cause of the 3/3 GCS deaths, 2026-07-09): ray.get_actor is on Ray's AUTO_INIT_APIS
        # list -- calling it BEFORE ray.init() silently boots a full throwaway local cluster
        # (gcs_server+raylet), which the build's ray.shutdown() then kills 5s later. That
        # init->shutdown->init sandwich on Ray's process-wide singletons left a client counting
        # 600s against the dead instance's address -> "Failed to connect to GCS" -> dead render.
        # ray.is_initialized() is NOT auto-init-wrapped (source-verified, Ray 2.30/2.34): guard first,
        # touch get_actor ONLY on an already-initialized Ray. Uninitialized => no actors can exist.
        try:
            if ray.is_initialized():
                ray.get_actor("RayWorker:0")                        # defense-in-depth: never rebuild over existing actors
                print("[Raylight][PRESTART] actors already present -> boot build skipped", flush=True)
                return
        except Exception:
            pass                                                    # no actors yet -> proceed to build
        cfg, n = _prestart_cfg_from_workflow()
        if n < 1:
            print("[Raylight][PRESTART] no CUDA devices visible -> skip", flush=True)
            return
        print(f"[Raylight][PRESTART] boot pre-start firing (GPU={n} FSDP={cfg.get('FSDP')})", flush=True)
        _t0 = _ps_time.time()
        RayInitializer().spawn_actor(**cfg)
        print(f"[Raylight][PRESTART] actors READY in {_ps_time.time() - _t0:.1f}s", flush=True)
        # F8: NEVER let this thread exit after a successful build. In 4/4 failed runs the driver's
        # GCS reconnect countdown opened at the INSTANT this thread returned (READY +0.1s) and never
        # recovered ("Failed to connect to GCS within Ns"); the identical build from ComfyUI's
        # long-lived executor thread has never failed. Parking a daemon thread costs nothing and is
        # harmless if the true mechanism lies elsewhere (Ray-source verification in flight).
        print("[Raylight][PRESTART] parking boot thread (F8: init-calling thread must outlive the session)", flush=True)
        _ps_th.Event().wait()
    except Exception:
        print("[Raylight][PRESTART] boot pre-start FAILED (non-fatal; warmup builds lazily):\n"
              + _ps_tb.format_exc(), flush=True)


# ============================================================================
# LOCAL PY_MODULES BYPASS (2026-07-13). Single-node cluster: Ray's py_modules zips, uploads and
# extracts the raylight package per session (visible as runtime_resources/py_modules_files/... in
# worker paths) so workers can import it — but the workers run on THIS machine and can import the
# installed tree directly. Drop py_modules and prepend the package parent to the workers'
# PYTHONPATH instead. Mutates the module-level _RAY_RUNTIME_ENV_LOCAL, which spawn_actor deepcopies
# on every build. VALIDATED 7/13 (worker 0lfvy7a3f9s9ld): actor build 44.2s -> 10.7s (ray.init
# 5.1 + spawn 5.6); workers import from the local tree (FSDP line shows custom_nodes path, no
# runtime_resources). Gate: default ON everywhere; RAYLIGHT_LOCAL_PYMODULES=0 is the kill-switch.
# ============================================================================
_lpm = os.environ.get("RAYLIGHT_LOCAL_PYMODULES", "auto")
if _lpm != "0":
    try:
        _lpm_parent = str(_RAYLIGHT_MODULE_PATH.parent)
        _lpm_ev = _RAY_RUNTIME_ENV_LOCAL.setdefault("env_vars", {})
        _lpm_pp = _lpm_ev.get("PYTHONPATH", "")
        if _lpm_parent not in _lpm_pp.split(os.pathsep):
            _lpm_ev["PYTHONPATH"] = _lpm_parent + (os.pathsep + _lpm_pp if _lpm_pp else "")
        _RAY_RUNTIME_ENV_LOCAL.pop("py_modules", None)
        print(f"[Raylight][LOCALPYMOD] py_modules dropped; worker PYTHONPATH += {_lpm_parent}", flush=True)
    except Exception as _lpm_e:
        print(f"[Raylight][LOCALPYMOD] failed non-fatally (py_modules kept): {_lpm_e!r}", flush=True)

# ray.init timing (log-only, always on): splits the actor-build wall (44.2s on 7/13's slow host,
# ~14s prod-class) into init vs actor-spawn on every future boot. Wrapping the attribute makes no
# call, so it cannot trip Ray's AUTO_INIT (F7).
# RAY-HEAD CONNECT (2026-07-14): when the handler pre-started a standalone Ray head at boot
# (marker /tmp/.ray_head_up), a local-cluster init is rewritten into a CONNECT (~1-2s vs ~6s
# cluster build). Namespace + runtime_env are kept (env_vars/PYTHONPATH apply on connect);
# new-cluster-only kwargs (object_store_memory, dashboard args) are dropped. Any connect failure
# falls back to the original local init — byte-path identical to today. Kill: RAY_HEAD_CONNECT=0.
_ORIG_RAY_INIT_TIMED = ray.init
_HEAD_TRIED = False   # ROUND-2 ONE-SHOT: exactly one connect attempt per process, ever. The 7/14
                      # crash-loop's plausible shape was reconnecting to a bad head on raylight's
                      # retry; this flag + marker deletion makes that structurally impossible.

def _consume_head_marker():
    """Read-and-delete /tmp/.ray_head_up: whatever happens next, no later init in ANY process can
    re-enter the connect path."""
    try:
        if os.path.exists("/tmp/.ray_head_up"):
            os.unlink("/tmp/.ray_head_up")
            return True
    except Exception:
        pass
    return False

def _timed_ray_init(*_a, **_kw):
    global _HEAD_TRIED
    _t = _ps_time.time()
    _mode = "local"
    try:
        _addr = _a[0] if _a else _kw.get("address")
        if (not _HEAD_TRIED
                and _addr in (None, "", "local", "LOCAL")
                and os.environ.get("RAY_HEAD_CONNECT", "1") == "1"
                and os.path.exists("/tmp/.ray_head_up")):
            _HEAD_TRIED = True
            _consume_head_marker()
            _ck = {k: v for k, v in _kw.items() if k in ("namespace", "runtime_env")}
            try:
                _ctx = _ORIG_RAY_INIT_TIMED(address="auto", **_ck)
                # VALIDATE the connected cluster before accepting it (7/14 hardening): a head with
                # wrong resources = broken actor spawns downstream. Reject -> shutdown -> local.
                _res = ray.cluster_resources()
                if _res.get("GPU", 0) >= 1 and _res.get("CPU", 0) >= 1:
                    _mode = "head_connect"
                    print(f"[Raylight][RAYHEAD] connected; cluster resources={_res}", flush=True)
                    return _ctx
                print(f"[Raylight][RAYHEAD] connected but BAD resources {_res}; shutdown -> local", flush=True)
                try: ray.shutdown()
                except Exception: pass
                _mode = "fallback_local_badres"
            except Exception as _he:
                print(f"[Raylight][RAYHEAD] connect failed ({_he!r}); falling back to local init", flush=True)
                try: ray.shutdown()
                except Exception: pass
                _mode = "fallback_local"
        return _ORIG_RAY_INIT_TIMED(*_a, **_kw)
    finally:
        print(f"[Raylight][TIMING] ray.init took {_ps_time.time() - _t:.1f}s ({_mode})", flush=True)

ray.init = _timed_ray_init


if os.environ.get("RAYLIGHT_PRESTART", "0") == "1":
    _ps_th.Thread(target=_prestart_boot, name="raylight-prestart", daemon=True).start()


# ============================================================================
# F9: STALL INSTRUMENTATION (2026-07-09). Heartbeat probes exposed a 20-78s SILENT stall between
# "got prompt" and "[Rank N] Applying FSDP", present on every resume/restart render, invisible in
# logs, uncorrelated with data movement (VRAM resident, shard load 2.5s). These wrappers bracket
# it from both sides so the NEXT run names it: (1) every ray.get > 2s logs its duration + call
# site; (2) every Raylight node function logs enter/exit + duration > 2s; (3) ensure_fresh_actors
# (dead-actor probe & rebuild decision) logs its verdict + duration. Print-only; no behavior
# change; runs in main process AND workers (both import this file). Gated on RAYLIGHT_TIMING!=0.
# ============================================================================
if os.environ.get("RAYLIGHT_TIMING", "1") != "0":
    _orig_ray_get = ray.get

    def _timed_ray_get(*_a, **_kw):
        _t = _ps_time.time()
        try:
            return _orig_ray_get(*_a, **_kw)
        finally:
            _d = _ps_time.time() - _t
            if _d > 2.0:
                try:
                    import sys as _sy
                    _f = _sy._getframe(1)
                    _site = f"{_ps_os.path.basename(_f.f_code.co_filename)}:{_f.f_lineno} in {_f.f_code.co_name}"
                except Exception:
                    _site = "?"
                print(f"[Raylight][TIMING] ray.get {_d:.1f}s at {_site}", flush=True)
    ray.get = _timed_ray_get

    def _wrap_timing(_cls, _name):
        _fn = getattr(_cls, _name, None)
        if _fn is None:
            return
        def _timed(self, *_a, **_kw):
            _t = _ps_time.time()
            print(f"[Raylight][TIMING] {_cls.__name__}.{_name} ENTER", flush=True)
            try:
                return _fn(self, *_a, **_kw)
            finally:
                print(f"[Raylight][TIMING] {_cls.__name__}.{_name} EXIT {_ps_time.time()-_t:.1f}s", flush=True)
        setattr(_cls, _name, _timed)

    for _c, _f in (("RayUNETLoader", "load_ray_unet"), ("XFuserKSamplerAdvanced", "ray_sample"),
                   ("UnifiedParallelSampler", "ray_sample"), ("DPKSamplerAdvanced", "ray_sample"),
                   ("RayVAELoader", "load_vae"), ("RayVAEDecodeDistributed", "ray_decode"),
                   ("RayLoraLoader", "load_lora")):
        try:
            _wrap_timing(globals()[_c], _f)
        except KeyError:
            pass

    try:
        _orig_efa = ensure_fresh_actors

        def _timed_efa(_init):
            _t = _ps_time.time()
            try:
                _r = _orig_efa(_init)
                return _r
            finally:
                print(f"[Raylight][TIMING] ensure_fresh_actors EXIT {_ps_time.time()-_t:.1f}s", flush=True)
        ensure_fresh_actors = _timed_efa
    except NameError:
        print("[Raylight][TIMING] ensure_fresh_actors not found (upstream rename?)", flush=True)
